...
function updateHospitalList(hospitals) {
    ...
    // Convert hospitals object to array for sorting
    const hospitalArray = Object.keys(hospitals).map(id => {
    ...
    const hospitalsToShow = hospitalArray.slice(0, systemSettings.maxHospitalsToShow);
...
function createHospitalCard(hospital) {
    ...
    if (capacityPercentage >= hospitalStatusThresholds.full) {
        capacityStatus = 'danger';
    } else if (capacityPercentage >= hospitalStatusThresholds.warning) {
    ...
    card.innerHTML = `
        ...
        <div class="hospital-actions">
            <button class="view-route-btn" data-hospital-id="${hospital.id}">View Route</button>
            <button class="call-hospital-btn" data-phone="${hospital.phone}">Call</button>
        </div>
    `;
