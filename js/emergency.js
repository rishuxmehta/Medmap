...
class EmergencyManager {
    ...
    showDriverNotification() {
        ...
        const driverAmbulance = document.getElementById('driver-ambulance');
        const driverLicense = document.getElementById('driver-license');
        const driverEta = document.getElementById('driver-eta');
        const driverMessage = document.querySelector('.driver-message p');

        if (this.assignedDriver) {
            if (driverName) driverName.textContent = this.assignedDriver.name;
            if (driverAmbulance) driverAmbulance.textContent = `Ambulance ID: ${this.assignedDriver.ambulanceId}`;
            if (driverLicense) driverLicense.textContent = `License: ${this.assignedDriver.license}`;
            if (driverEta) driverEta.textContent = this.assignedDriver.eta;
            if (driverMessage) driverMessage.textContent = this.assignedDriver.message;
        }

        // Show notification with animation
        this.driverNotification.style.display = 'block';
        setTimeout(() => {
            this.driverNotification.classList.add('active');
        }, 10);

        // Add event listeners for buttons
        this.setupDriverNotificationButtons();

        // Auto-hide after 30 seconds
        setTimeout(() => {
            if (this.driverNotification.classList.contains('active')) {
                this.hideDriverNotification();
            }
        }, 30000);
    }
    ...
    /**
     * Setup driver notification buttons
     */
    setupDriverNotificationButtons() {
        // Call driver button
        if (this.callDriverBtn) {
            this.callDriverBtn.onclick = () => {
                this.callDriver();
                this.hideDriverNotification();
            };
        }
        ...
        if (this.trackAmbulanceBtn) {
            this.trackAmbulanceBtn.onclick = () => {
                this.trackAmbulance();
                this.hideDriverNotification();
            };
        }
        ...
        // Close button
        if (this.closeDriverNotificationBtn) {
            this.closeDriverNotificationBtn.onclick = () => {
                this.hideDriverNotification();
            };
        }
    }
    ...
    hideDriverNotification() {
        ...
        this.driverNotification.classList.remove('active');

        // Hide after animation
        setTimeout(() => {
            this.driverNotification.style.display = 'none';
        }, 300);
    }
     ...
     */
    callDriver() {
        if (!this.assignedDriver || !this.assignedDriver.phone) {
            console.error('Driver phone number not available');
            return;
        }

        // Create phone link
        const phoneLink = document.createElement('a');
        phoneLink.href = `tel:${this.assignedDriver.phone}`;
        phoneLink.click();
    }
     ...
     */
    trackAmbulance() {
        if (!this.assignedDriver || !this.assignedDriver.ambulanceId) {
            console.error('Ambulance ID not available');
            return;
        }
        ...
        // Update map to show ambulance location
        // This would be implemented in map.js
        if (typeof updateMapWithAmbulance === 'function') {
            updateMapWithAmbulance(this.assignedDriver.ambulanceId);
        }
    }
}
